//
//  TRZXFriendLineRefreshFooter.m


#import "TRZXFriendLineRefreshFooter.h"
#import "UIView+SDAutoLayout.h"

#define kTRZXFriendLineRefreshFooterHeight 50

@implementation TRZXFriendLineRefreshFooter

+ (instancetype)refreshFooterWithRefreshingText:(NSString *)text
{
    TRZXFriendLineRefreshFooter *footer = [TRZXFriendLineRefreshFooter new];
...
- (void)setRefreshState:(SDWXRefreshViewState)refreshState
{
    ...
    if (refreshState == SDWXRefreshViewStateRefreshing) {
        ...
        insets.bottom += kTRZXFriendLineRefreshFooterHeight;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    ...
    if (self.scrollView.contentOffset.y > self.scrollView.contentSize.height - self.scrollView.height && self.refreshState != SDWXRefreshViewStateRefreshing) {
        self.frame = CGRectMake(0, self.scrollView.contentSize.height, self.scrollView.width, kTRZXFriendLineRefreshFooterHeight);
        self.hidden = NO;
