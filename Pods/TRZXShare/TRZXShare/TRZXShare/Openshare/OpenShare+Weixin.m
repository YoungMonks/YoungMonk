...
+(void)connectWeixinWithAppId:(NSString *)appId secret:(NSString *)secret{
    [self set:schema Keys:@{@"appid":appId,@"secretKey":secret}];
...
+ (void)weixinOAuthWithMessage:(NSDictionary *)message completionHandle:(void (^)(NSDictionary *, NSError *))completionHandler
{
    ...
    NSString *appId = [self keyFor:schema][@"appid"];
    NSString *secret = [self keyFor:schema][@"secretKey"];
    ...
    [self get:url completionHandler:^(NSDictionary *data, NSError *error) {
        ...
        [self get:userInfoUrl completionHandler:^(NSDictionary *userInfo, NSError *error) {
