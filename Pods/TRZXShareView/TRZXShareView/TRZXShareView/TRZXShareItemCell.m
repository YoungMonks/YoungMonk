...
@implementation TRZXShareItemCell
...
- (void)layoutSubviews
{
    ...
    // 图标
    CGFloat iconViewX = TRZX_ItemCellPadding / 2;
    CGFloat iconViewY = topPadding;
    CGFloat iconViewW = cellWidth - TRZX_ItemCellPadding;
    CGFloat iconViewH = cellWidth - TRZX_ItemCellPadding;
...
- (void)iconClick
{
    if (self.item.selectionHandler) {
        [[NSNotificationCenter defaultCenter] postNotificationName:TRZX_HideNotification object:nil];
...
- (void)setItem:(TRZXShareItem *)item
{
    _item = item;
